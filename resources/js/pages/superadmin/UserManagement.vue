<template>
  <div>
    ...
    <div class="flex justify-between items-center mb-6">
      ...
      <Button class="px-4 py-2" @click="showAddModal = true">Add User</Button>
    </div>
          ...
          <TableRow v-for="user in paginatedUsers" :key="user.id" class="hover:bg-gray-50 transition-colors">
            ...
            <TableCell>
              <span
                ...
                :class="user.role === 'SUPERADMIN' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-blue-700'"
              >
                {{ user.role === 'SUPERADMIN' ? 'Superadmin' : 'Office Admin' }}
              </span>
            </TableCell>
            <TableCell class="text-gray-600">
              {{ user.role === 'SUPERADMIN' ? '—' : user.office }}
            </TableCell>
      ...
      <Transition name="modal-fade">
        <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 z-10 mx-4">
            ...
            <h2 class="text-2xl font-bold text-[#164980] mb-6">Add User</h2>
            ...
            <div class="flex justify-end gap-3">
              ...
              <Button class="px-5 py-2" @click="handleAddUser">Add User</Button>
            </div>
          </div>
        </div>
      </Transition>
      ...
      <Transition name="modal-fade">
        <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 z-10 mx-4">
            ...
            <h2 class="text-2xl font-bold text-[#164980] mb-6">Edit User</h2>
            ...
            <div class="space-y-4 mb-8">
              ...
              <div v-if="editUser.role === 'OFFICE ADMIN'">
                <label class="block text-sm text-gray-700 mb-1">Office:</label>
                <select v-model="editUser.office"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-[#164980] focus:border-transparent transition">
                  <option value="" disabled>Select an office</option>
                  <option v-for="office in offices" :key="office" :value="office">{{ office }}</option>
                </select>
            ...
            <div class="flex justify-end gap-3">
              ...
              <Button class="px-5 py-2" @click="handleSaveUser">Save</Button>
            </div>
          </div>
        </div>
      </Transition>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 z-10 mx-4">
            ...
            <div class="flex justify-end gap-3">
              ...
              <button @click="handleDeleteUser" class="px-5 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-medium cursor-pointer">Delete</button>
            </div>
          </div>
        </div>
      </Transition>
...
import {
  SquarePen, Trash2, X, MoreHorizontal, Eye, EyeOff
} from 'lucide-vue-next'

// ---- HARDCODED DATA ----
const offices = [
  'Office of the City Mayor (CMO)',
  'Office of the City Mayor-Library Services (CMO-LS)',
  'Office of the City Mayor-Ligao Community College (CMO-LCC)',
  'City General Services Office (CGSO)',
  'Office of the City Local Civil Registrar (CLCR)',
  "City Treasurer's Office (CTO)",
  "Office of the City Treasurer's Office-Operation Economic Enterprise (CTO-OEE)",
  "City Assessor's Office (CAO)",
  'Business Processing Licensing Office (BPLO)',
]

const users = ref([
  { id: 1, username: 'superadmin', role: 'SUPERADMIN', office: null },
  { id: 2, username: 'cmo_admin', role: 'OFFICE ADMIN', office: 'Office of the City Mayor (CMO)' },
  { id: 3, username: 'cgso_admin', role: 'OFFICE ADMIN', office: 'City General Services Office (CGSO)' },
  { id: 4, username: 'clcr_admin', role: 'OFFICE ADMIN', office: 'Office of the City Local Civil Registrar (CLCR)' },
  { id: 5, username: 'cto_admin', role: 'OFFICE ADMIN', office: "City Treasurer's Office (CTO)" },
  { id: 6, username: 'cao_admin', role: 'OFFICE ADMIN', office: "City Assessor's Office (CAO)" },
  { id: 7, username: 'bplo_admin', role: 'OFFICE ADMIN', office: 'Business Processing Licensing Office (BPLO)' },
])
...
const newUser = ref({ username: '', password: '', role: 'OFFICE ADMIN', office: '' })

const closeAddModal = () => {
  showAddModal.value = false
  showNewPassword.value = false
  newUser.value = { username: '', password: '', role: 'OFFICE ADMIN', office: '' }
}
const handleAddUser = () => {
...
const editUser = ref({ username: '', password: '', role: 'OFFICE ADMIN', office: '' })
const userToEdit = ref(null)

const openEditModal = (user) => {
  userToEdit.value = user
  editUser.value = { username: user.username, password: '', role: user.role, office: user.office || '' }
...
const handleDeleteUser = () => {
  console.log('Soft-deleting user:', userToDelete.value?.username)
  closeDeleteModal()
}
</script>
