  ...
  <div>
    ...
    <div class="flex items-center gap-2 text-sm mb-6">
      <button
        ...
        class="text-[#164980] hover:underline font-medium cursor-pointer"
      >
        Office Management
      </button>
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <Button class="px-4 py-2" @click="showAddModal = true">Add Service</Button>
    </div>
          ...
          <TableRow v-for="service in paginatedServices" :key="service.id" class="hover:bg-gray-50 transition-colors">
            ...
            <TableCell class="text-center">
              <span
                ...
                :class="service.is_free ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'"
              >
            ...
            <TableCell class="text-center">
              <span
                class="px-2 py-0.5 rounded-full text-xs font-semibold"
                :class="service.status === 'Active' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'"
              >
                {{ service.status }}
              </span>
            </TableCell>
    ...
    <div class="flex items-center justify-between mt-4">
      <span class="text-sm text-gray-500">
        Showing {{ (currentPage - 1) * pageSize + 1 }}–{{ Math.min(currentPage * pageSize, services.length) }} of {{ services.length }} services
      </span>
      <div class="flex items-center gap-1">
        ...
        <button
          ...
          :class="currentPage === page ? 'bg-[#164980] text-white border-[#164980]' : 'border-gray-200 text-gray-600 hover:bg-gray-50'"
        >
          {{ page }}
        </button>
        <button
      ...
      <Transition name="modal-fade">
        <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 z-10 mx-4">
            ...
            <h2 class="text-2xl font-bold text-[#164980] mb-6">Add Service</h2>
            ...
            <div class="space-y-4 mb-8">
              ...
              <div>
                ...
                <select v-model="newService.is_free"
            ...
            <div class="flex justify-end gap-3">
              <button @click="closeAddModal" class="px-5 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm font-medium cursor-pointer">Cancel</button>
              <Button class="px-5 py-2" @click="handleAddService">Save</Button>
            </div>
          </div>
        </div>
      </Transition>
      ...
      <Transition name="modal-fade">
        <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 z-10 mx-4">
            ...
            <h2 class="text-2xl font-bold text-[#164980] mb-6">Edit Service</h2>
            ...
            <div class="space-y-4 mb-8">
              ...
              <div>
                <label class="block text-sm text-gray-700 mb-1">Is Free:</label>
                <select v-model="editService.is_free"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-[#164980] focus:border-transparent transition">
                  <option :value="true">Yes</option>
                  <option :value="false">No</option>
                </select>
            ...
            <div class="flex justify-end gap-3">
              <button @click="closeEditModal" class="px-5 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm font-medium cursor-pointer">Cancel</button>
              <Button class="px-5 py-2" @click="handleSaveService">Save</Button>
            </div>
          </div>
        </div>
      </Transition>
      ...
      <Transition name="modal-fade">
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 z-10 mx-4">
            ...
            <div class="flex justify-end gap-3">
              <button @click="closeDeleteModal" class="px-5 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm font-medium cursor-pointer">Cancel</button>
              <button @click="handleDeleteService" class="px-5 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-medium cursor-pointer">Delete</button>
            </div>
          </div>
        </div>
      </Transition>
...
import {
  ChevronRight, SquarePen, Trash2, X, MoreHorizontal, Lock, LockOpen
...
// ---- SERVICES DATA ----
const services = ref([
  { id: 1, name: 'Medical Consultation', code: 'MED-001', is_free: true, used_count: 45, status: 'Active', is_locked: true },
  { id: 2, name: 'Dental Check-Up', code: 'DEN-001', is_free: true, used_count: 22, status: 'Active', is_locked: true },
  { id: 3, name: 'Laboratory Test', code: 'LAB-001', is_free: false, used_count: 10, status: 'Active', is_locked: false },
  { id: 4, name: 'X-Ray', code: 'RAD-001', is_free: false, used_count: 0, status: 'Active', is_locked: false },
  { id: 5, name: 'Vaccination', code: 'VAC-001', is_free: true, used_count: 80, status: 'Active', is_locked: true },
  { id: 6, name: 'Blood Pressure Monitoring', code: 'BPM-001', is_free: true, used_count: 0, status: 'Inactive', is_locked: false },
  { id: 7, name: 'Prenatal Check-Up', code: 'PRE-001', is_free: true, used_count: 33, status: 'Active', is_locked: true },
  { id: 8, name: 'Family Planning Counseling', code: 'FPL-001', is_free: true, used_count: 5, status: 'Active', is_locked: false },
])
...
const paginatedServices = computed(() =>
  services.value.slice((currentPage.value - 1) * pageSize, currentPage.value * pageSize)
)
...
const newService = ref({ name: '', code: '', is_free: true })

const closeAddModal = () => {
  showAddModal.value = false
  newService.value = { name: '', code: '', is_free: true }
}
...
const editService = ref({ name: '', code: '', is_free: true })
const serviceToEdit = ref(null)

const openEditModal = (service) => {
  if (service.is_locked) return
  serviceToEdit.value = service
  editService.value = { name: service.name, code: service.code, is_free: service.is_free }
